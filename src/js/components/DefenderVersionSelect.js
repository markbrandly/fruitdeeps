        ...
        if (this.props.monList.length < 5) {
            return (
                <div>
						...
						<button value={i} onClick={this.clickHandler} className={listItem.version == this.props.monster.version && listItem.version_number === this.props.monster.version_number && listItem.combat === this.props.monster.combat ? "selected" : ""}>{listItem.version || (i+1)}</button>
						)}
					</div>
            )
        }
