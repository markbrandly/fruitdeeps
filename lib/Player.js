...
export default class Player {
    ...
    equip(item) {
        ...
        if (
            (item.slot == "weapon" || item.slot == "2h") &&
            item.category != this.equipment.weapon.category
        ) {
    ...
    selectPrayer(prayer) {
        ...
        if (
            prayerList.ranged.includes(prayer) ||
            prayerList.magic.includes(prayer) ||
            prayerList.melee.includes(prayer)
        ) {
            this.clearPrayers();
    ...
    toggleCharge() {
        this.misc.charge = this.misc.charge ? false : true;
    }
    ...
    get bonusesRaw() {
        ...
        if (
            this.equipment.weapon.name.includes("Crystal bow") ||
            this.equipment.weapon.name.includes("Craw's bow") ||
            this.equipment.weapon.category === "Thrown" ||
            this.equipment.weapon.category === "Chinchompas"
        ) {
    ...
    get combat() {
        var base =
            ...
            (this.stats.defence +
                ...
                Math.floor(this.stats.prayer / 2));
    ...
    minimize() {
        ...
        if (fullObj.setName !== null) {
            minObj.setName = fullObj.setName;
        }
