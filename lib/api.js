...
const addNpc = async (npc, fn) => {
    ...
    //Insert / update npc
    return con("monster").insert({
        name: npc.name,
        version: npc.version,
        imageName: npc.image,
        combat: parseInt(npc.combat) || 0,
        version_number: parseInt(npc.versionNumber) || null,
    })
    .onConflict()
    .merge()
    .then(() => {
    ...
    .then(async (rows) => {
        ...
        let bonuses = Object.keys(npc.stats)
        for(let i = 0; i < 17; i++){
            let bonus = bonuses[i]
            await con("monster_stat")
            .insert({
                monsterId: rows[0].idmonster,
                category: bonus,
                value: parseInt(npc.stats[bonus]) || 0,
            })
            .onConflict()
            .merge()
        }
        ...
        for(let i = 0; i < npc.attributes.length; i++){
            await con("monster_attributes")
            .insert({
                monsterId: rows[0].idmonster,
                attribute: npc.attributes[i]
            })
            .onConflict()
            .merge()
        }
...
export default { getMonstersByName, searchMonsterNames, searchItems, addItem, addNpc };
